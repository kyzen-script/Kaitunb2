-- ===== CONFIG: Khai bao Config mac dinh neu chua ton tai =====
...
-- ===== Khai bao bien r (session timer) =====
if not getgenv()._r_session then
    getgenv()._r_session = {nil, "SessionKey", {}}
    getgenv()._r_session[3][getgenv()._r_session[2]] = 0
end
local r = getgenv()._r_session

-- ===== Khai bao bien k tuong tu r =====
if not getgenv()._k_session then
    getgenv()._k_session = {nil, "SessionKey", {}}
    getgenv()._k_session[3][getgenv()._k_session[2]] = false
end
local k = getgenv()._k_session

-- ===== Khai bao ham Report =====
function Report(msg)
    pcall(function()
        warn("[Kyzen Kaitun Report] " .. tostring(msg))
    end)
end

-- ===== Khai bao ham LockAimPositionTo =====
function LockAimPositionTo(pos)
    pcall(function()
...
-- ===== Khai bao SCRIPT_CONFIG =====
if not getgenv().SCRIPT_CONFIG then
...
function hoangtuveu()
    ...
    local _uiStroke1 = Instance.new("UIStroke")
    ...
    function J:SetIntensity(C)
        ...
        z:Play()
        for _bIdx, _bObj in pairs(b) do
            if _bObj and _bObj.Parent then
                local v = p:Create(_bObj, o, {BackgroundTransparency = _bObj._originalTransparency + (C * 0.5)})
                v:Play()
            end
        end
    end
    ...
    function ToggleUI(J)
        ...
        for _iKey, _iVal in pairs(W.Instances) do table.insert(J, _iVal) end
        ...
        if a then
            ...
            for _idx, _elem in pairs(J) do
                _elem.TextTransparency = 1
                local o = X:Create(_elem, C, {TextTransparency = 0})
                if _elem:FindFirstChildOfClass('UIStroke') then
                    _elem:FindFirstChildOfClass('UIStroke').Transparency = 1
                    local z = X:Create(_elem:FindFirstChildOfClass('UIStroke'), C, {Transparency = 0})
                    z:Play()
                end
                o:Play()
            end
            ...
            for _idx2, _elem2 in pairs(J) do
                local _tw = X:Create(_elem2, C, {TextTransparency = 1})
                if _elem2:FindFirstChildOfClass('UIStroke') then
                    local o = X:Create(_elem2:FindFirstChildOfClass("UIStroke"), C, {Transparency = 1})
                    o:Play()
                end
                _tw:Play()
            end
    ...
    K.AnimationId = 'http://www.roblox.com/asset/?id=0'
    ...
    repeat
        task.wait()
        game.ReplicatedStorage.Remotes.CommF_:InvokeServer("SetTeam", getgenv().Config.Team)
    ...
    Services = {}
    setmetatable(Services, {__index = function(_self, _key) return game:GetService(_key) end})
    setmetatable(ScriptStorage.Enemies, {__index = function(_self, _key) return Services.Workspace.Enemies:FindFirstChild(_key) or Services.ReplicatedStorage:FindFirstChild(_key) end})
    setmetatable(ScriptStorage.Map, {__index = function(_self, _key) return Services.Workspace.Map:FindFirstChild(_key) or Services.Workspace:FindFirstChild(_key) end})
    setmetatable(ScriptStorage.Tools, {__index = function(_self, _key) return LocalPlayer.Character:FindFirstChild(_key) or LocalPlayer.Backpack:FindFirstChild(_key) end})
    setmetatable(ScriptStorage.NPCs, {__index = function(_self, _key) if not _key then return end; return workspace.NPCs:FindFirstChild(_key) or game.ReplicatedStorage.NPCs:FindFirstChild(_key) end})
    ...
    local J = {}
    setmetatable(Remotes, {__index = function(_self, _remoteKey)
        if _remoteKey ~= 'CommF_' then
            print('captured unregistered signal', _remoteKey)
            return Services.ReplicatedStorage.Remotes[_remoteKey]
        end
        local _commProxy = {InvokeServer = function(_proxyself, ...)
            print('remote fired', ...)
            local a, h = ...
    ...
    function AddPoint()
        local W = {}
        local a
        for _si, _stat in LocalPlayer.Data.Stats:GetChildren() do
            if _stat and _stat:FindFirstChild('Level') then W[_stat.Name] = _stat.Level.Value end
        end
    ...
    function RefreshPlayerData()
        for _di, _dChild in LocalPlayer.Data:GetChildren() do pcall(function() ScriptStorage.PlayerData[_dChild.Name] = _dChild.Value end) end
        local a = ""
        for _pdKey, _pdVal in ScriptStorage.PlayerData do
            local _pdColor = W.Currencies[_pdKey]
            if _pdColor then a = a .. '<font color="' .. _pdColor .. '">' .. _pdKey .. "</font>: " .. _pdVal .. ' ' end
        end
    ...
    function RefreshInventory()
        ...
        for _invIdx, _invItem in Remotes.CommF_:InvokeServer('getInventory') do ScriptStorage.Backpack2[_invItem.Name] = _invItem end
    ...
    function RefreshMelees(W)
        local a = ''
        for _mName, _mLevel in ScriptStorage.Melees do a = a .. _mName .. ": " .. _mLevel .. " " end
        a = a == '' and '[0]' or a
    ...
    function RegisterLocalPlayerEventsConnection()
        ...
        for _ck, _cv in ScriptStorage.Connections.LocalPlayer do pcall(function() _cv:Disconnect() end) end
        ...
        table.foreach(LocalPlayer.Backpack:GetChildren(), function(_mi, _mTool) MeleeCheck(_mTool) end)
    ...
    function GetMonAsSortedRange()
        local W = {}
        table.foreach(Services.Workspace.Enemies:GetChildren(), function(_ei, _enemy)
            if _enemy and _enemy:FindFirstChild('Humanoid') and _enemy:FindFirstChild("HumanoidRootPart") and _enemy.Humanoid.Health > 0 then
                table.insert(W, _enemy)
            end
        end)
        table.foreach(game.ReplicatedStorage:GetChildren(), function(_ri, _rEnemy)
            if _rEnemy and _rEnemy:FindFirstChild('Humanoid') and _rEnemy:FindFirstChild("HumanoidRootPart") and _rEnemy.Humanoid.Health > 0 then
                table.insert(W, _rEnemy)
            end
        end)
    ...
    print(1.5)
    function GetMeleeIdByName(W) for _mIdx, _mName in MeleesTable do if _mName == W then return MeleesId[_mIdx] end end end
    function getpos(W)
        for _ni, _npc in game:GetService("ReplicatedStorage").NPCs:GetChildren() do if _npc.Name == W then return _npc.HumanoidRootPart.CFrame end end
        for _ni2, _npc2 in workspace.NPCs:GetChildren() do if _npc2.Name == W then return _npc2.HumanoidRootPart.CFrame end end
    end
    function BuyMelee(W, a)
        if W == "DragonClaw" and workspace.NPCs:FindFirstChild('Sabi') then
            if a then
                local _dcResult = Remotes.CommF_:InvokeServer("BlackbeardReward", 'DragonClaw', '1')
                if type(_dcResult) == "number" and _dcResult == 1 and not table.find(J, W) then table.insert(J, W) end
    ...
    function J.RefreshQuest(W)
        ...
        local a = 0
        local h
        for _qId, _qData in J.Quests do
            if not J.BlacklistedQuestIds[_qId] then
                if (_qData[1].LevelReq >= a and _qData[1].LevelReq <= ScriptStorage.PlayerData.Level) then
                    a = _qData[1].LevelReq
                    h = _qData
                    W.CurrentQuestId = _qId
                    if ScriptStorage.PlayerData.Level >= 1500 and SeaIndex == 2 and _qId == 'ForgottenQuest' then break end
                end
            end
        end
        local a = h[#h]
        for _taskKey, _taskVal in a.Task do if _taskVal == 1 then table.remove(h, #h) end end
        for _npcData, _npcInfo in require(game.ReplicatedStorage.GuideModule).Data.NPCList do
            for _lvlIdx, _lvlVal in _npcInfo.Levels do if _lvlVal == h[#h].LevelReq then W.CurrentNpc = _npcData.CFrame end end
        end
        W.CurrentQuests = h
    end
    ...
    spawn(function()
    pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/sucvatthieunang/Trackstat/main/cac"))()
    end)
end)

-- Đã xoá link boss chết 404

ScriptStorage.MobRegions = {}

for _spIdx, _spChild in game:GetService("ReplicatedStorage").FortBuilderReplicatedSpawnPositionsFolder:GetChildren() do
    ScriptStorage.MobRegions[tostring(_spChild)] = ScriptStorage.MobRegions[tostring(_spChild)] or {}
    table.insert(ScriptStorage.MobRegions[tostring(_spChild)], _spChild.CFrame)
end
    TweenController = {}
    local W = 0
    local W = {}
    for _hpIdx, _hpNpc in game.ReplicatedStorage.NPCs:GetChildren() do if _hpNpc.Name == 'Set Home Point' then table.insert(W, _hpNpc:GetModelCFrame()) end end
    ...
    function GetPortal(a)
        local h, X = 9e9, nil
        for _pIdx, _pPos in Portals do
            local D = CaculateDistance(_pPos, a)
    ...
    function GetEntries(a)
        local h, X = 9e9, nil
        for _geIdx, _gePos in W do
            local _geDist = CaculateDistance(_gePos, a)
            if _geDist < (CaculateDistance(a) - 700) and _geDist < h then
                h = _geDist
                X = _gePos
            end
        end
    ...
    function TweenController.Create(W)
        ...
        for _descIdx, _descPart in ipairs(game.Players.LocalPlayer.Character:GetDescendants()) do if _descPart:IsA("BasePart") then _descPart.CanCollide = false end end
    ...
    function GetAllBladeHits()
        bladehits = {}
        for _bhIdx, _bhMob in pairs(workspace.Enemies:GetChildren()) do
            if _bhMob:FindFirstChild('Humanoid') and _bhMob:FindFirstChild('HumanoidRootPart') and _bhMob.Humanoid.Health > 0 and (_bhMob.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 65 then
                table.insert(bladehits, _bhMob)
            end
        end
        return bladehits
    end
    function Getplayerhit()
        bladehits = {}
        for _phIdx, _phChar in pairs(workspace.Characters:GetChildren()) do
            if _phChar.Name ~= game.Players.LocalPlayer.Name and _phChar:FindFirstChild('Humanoid') and _phChar:FindFirstChild('HumanoidRootPart') and _phChar.Humanoid.Health > 0 and (_phChar.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 65 then
                table.insert(bladehits, _phChar)
            end
        end
        return bladehits
    end
    ...
    function h:Attack()
        local X = {}
        for _aIdx, _aMob in pairs(GetAllBladeHits()) do table.insert(X, _aMob) end
        for _aIdx2, _aPlr in pairs(Getplayerhit()) do table.insert(X, _aPlr) end
        if #X == 0 then return end
        local y = {[1] = nil, [2] = {}, [4] = "078da5141"}
        for _hitIdx, _hitTarget in pairs(X) do
            w:FireServer(0)
            if not y[1] then y[1] = _hitTarget.Head end
            table.insert(y[2], {[1] = _hitTarget, [2] = _hitTarget.HumanoidRootPart})
            table.insert(y[2], _hitTarget)
        end
    ...
    function CombatController.Grab(h)
        ...
        ForcePosition = nil
        local D = {}
        for _eIdx, _eMob in Services.Workspace.Enemies:GetChildren() do
            if _eMob.Name == h then
                if _eMob:FindFirstChild('Humanoid') and _eMob:FindFirstChild("HumanoidRootPart") and _eMob.Humanoid.Health > 0 then
                    local _ePos = _eMob.HumanoidRootPart.Position
                    if _ePos and isnetworkowner(_eMob.PrimaryPart) then
                        if not ForcePosition or CaculateDistance(_ePos, ForcePosition) < CombatController.GRAB_DISTANCE then
                            w = w + 1
                            _eMob:SetAttribute("OldPosition", _eMob:GetAttribute('OldPosition') or _ePos)
                            X = X + _ePos
                            ForcePosition = ForcePosition or _ePos
                            table.insert(D, _eMob)
                        end
                    end
                end
            end
        ...
        X = CFrame.new(X / w)
        table.foreach(D, function(_gIdx, _gMob)
            (function()
                if _gMob:GetAttribute("IgnoreGrab") then return end
                if (_gMob:GetAttribute("FailureCount") or 0) > 7 then return end
                local w = _gMob:FindFirstChild("HumanoidRootPart")
                ...
                _gMob:SetAttribute('IsGrabbed', true)
                _gMob.HumanoidRootPart.CFrame = X
                _gMob:SetAttribute("MidPoint", X)
            end)()
        end)
    end
    ...
    function CombatController.Search(h)
        local X = {}
        local w = false
        for _sIdx, _sMob in GetMonAsSortedRange() do
            if table.find(h, _sMob.Name) and _sMob:FindFirstChild("Humanoid") and _sMob.Humanoid.Health > 0 then
                if (_sMob:GetAttribute('FailureCount') or 0) < 3 then
                    w = true
                    table.insert(X, _sMob)
                end
            end
        end
        ...
        for _sIdx2, _sName in h do
            local _sFound = game.ReplicatedStorage:FindFirstChild(_sName)
            if _sFound then return _sFound end
        end
    end
        ...
        for y, L in (h) do
            ...
            elseif not X then
                if (os.time() - LastFound) > 200 then
                    alert('Kyzen Kaitun', 'Error while farming, rejoin')
                    game.Players.LocalPlayer:Kick('Rejoining..')
                    return
                end
    ...
    FunctionsHandler.ExpRedeem:RegisterMethod("Start", function()
        ...
        for _codeIdx, _codeName in h do
            SetTask("MainTask", 'Code Redemption | ' .. _codeName .. ' | Redeeming...')
            local _codeResult = pcall(function() return Services.ReplicatedStorage.Remotes.Redeem:InvokeServer(_codeName) end)
            task.wait()
            SetTask('MainTask', 'Code Redemption | ' .. _codeName .. " | " .. (tostring(_codeResult) or "Failed"))
            if getsenv(game.ReplicatedStorage.GuideModule)._G.ServerData.ExpBoost == 0 then
                if _codeResult and tostring(_codeResult) and string.find(tostring(_codeResult), 'SUCC') then
    ...
    FunctionsHandler.LevelFarm:RegisterMethod("Start", function(h)
            ...
            local w, D = (function()
                ...
                for y, L in GodhumanMaterials do
                    if (ScriptStorage.Backpack[y] or {Count = 0}).Count < L[1] then
                        getgenv()["_mphm_cache"] = {y, L}
                    end
                end
                return unpack(getgenv()["_mphm_cache"])
            end)()
            if w then
        ...
        if os.time() - LastTravel > 60 then
            ...
            if X >= 1500 and SeaIndex == 2 then
                if getgenv().Config.Settings.StayInSea2UntilHaveDarkFragments and not ScriptStorage.Backpack['Dark Fragment'] then
    ...
    FunctionsHandler.Saber:RegisterMethod('Refresh', function()
        if not getgenv().Config.Items.Saber then return end
        if not getgenv().Config.Items.Saber then return end
        local h
        ...
        for _plateIdx, _plateVal in X.Plates do if _plateVal == false then h = 1 end end
        if not h then
    ...
    FunctionsHandler.Saber:RegisterMethod('GetQuestplates', function()
        ...
        local X = {}
        table.foreach(h.QuestPlates:GetChildren(), function(_qpIdx, _qpPlate) if _qpPlate:FindFirstChild("Button") then table.insert(X, _qpPlate) end end)
    ...
    pcall(function() Remotes.RefreshQuestPro.OnClientEvent:Connect(FunctionsHandler.Saber.Methods.Refresh.Callback) end)
    MeleeLastCursor = 1
    FirstCall = true
    CanPurchase = {}
    FunctionsHandler.MeleesController:RegisterMethod('Start', function()
        for h, X in MeleesTable do
            if X ~= "SanguineArt" then
                if not getgenv().Config.Items.AutoFullyMelees then break end
                Data = MeleePrices[X]
        ...
        FarmingItem = nil
        for _bpKey2, _bpItem2 in ScriptStorage.Backpack do
            if _bpItem2.Type == "Sword" then
                if _bpItem2.Name == 'Yama' or _bpItem2.Name == "Tushita" then
                    MasteryRequirement = 350
                else
                    for _mrIdx, _mrVal in _bpItem2.MasteryRequirements do MasteryRequirement = _mrVal end
                end
                if _bpItem2.Mastery < MasteryRequirement then
                    if _bpItem2.Name == "Yama" or _bpItem2.Name == "Tushita" then
                        FarmingItem = {_bpItem2.Name, _bpItem2.Mastery, MasteryRequirement}
                        break
                    end
                end
            end
        ...
        -- k[3][k[2]] = true -- Luu trang thai melee completed
    end)
    ...
    FunctionsHandler.EvoRace:RegisterMethod("Refresh", function()
        if not getgenv().Config.Items.RaceV2 then return end
        if SeaIndex ~= 2 then return end
    ...
    FunctionsHandler.BossesTask:RegisterMethod("Refresh", function()
        local k
        for _boIdx, _boName in BossesOrder do
            local _boLevel = BossesOrderLevel[_boName]
            if ScriptStorage.PlayerData.Level >= _boLevel then
                local _boBoss = ScriptStorage.Enemies[_boName]
                if _boBoss and _boBoss:FindFirstChild("Humanoid") and _boBoss.Humanoid.Health > 0 then k = _boBoss end
            end
        end
    ...
    FunctionsHandler.SpecialBossesTask:RegisterMethod('Refresh', function()
        local k
        for _sbName, _sbLevel in SpecialBossesOrder do
            if ScriptStorage.PlayerData.Level >= _sbLevel then
                local _sbBoss = ScriptStorage.Enemies[_sbName]
                if _sbBoss and _sbBoss:FindFirstChild('Humanoid') and _sbBoss.Humanoid.Health > 0 then k = _sbBoss end
            end
        end
        return k
    end)
    ...
    FunctionsHandler.RaidController:RegisterMethod('RefreshRaidType', function()
        for _raidIdx, _raidName in require(game.ReplicatedStorage.Raids).raids do
            if string.find(ScriptStorage.PlayerData.DevilFruit, _raidName) then
                FunctionsHandler.RaidController:Set('CurrentChip', _raidName)
                return
            end
        end
    ...
    FunctionsHandler.RaidController:RegisterMethod('GetRaidableFruit', function()
        for _bpKey, _bpItem in ScriptStorage.Backpack do
            if string.find(FruitIdToName(_bpItem.Name), " Fruit") then
                if _bpItem.Value and _bpItem.Value < 1000000 then return _bpItem end
            end
        end
    end)
    FunctionsHandler.RaidController:RegisterMethod("GetCurrentRaidIsland", function()
        ...
        IslandsList = {{}, {}, {}, {}, {}}
        for _locIdx, _loc in workspace['_WorldOrigin'].Locations:GetChildren() do
            if string.find(_loc.Name, 'Island ') and CaculateDistance(_loc.Position, Vector3.new(0, 0, 0)) > 7000 then
                (function()
                    local h = string.gsub(_loc.Name, "Island ", "")
                    local X = tonumber(h)
                    table.insert(IslandsList[X], _loc)
                end)()
            end
        end
        do
            for k = 5, 1, -1.0 do
                for _isIdx, _island in IslandsList[k] do if CaculateDistance(_island.Position) < 2000 then return _island end end
            end
        end
    end)
    function CheckSpecialMicrochip()
        for _tblIdx, _tbl in ipairs({LocalPlayer.Character:GetChildren(), LocalPlayer.Backpack:GetChildren()}) do
            for _itemIdx, _item in ipairs(_tbl) do if _item.Name == "Special Microchip" then return _item end end
        end
    end
    ...
    FunctionsHandler.RaidController:RegisterMethod("Start", function()
        ...
        if not k then
            ...
            local h = false
            for _rmIdx, _rmMob in GetMonAsSortedRange() do
                local w = os.time()
                while _rmMob and _rmMob:FindFirstChild("HumanoidRootPart") and _rmMob.Humanoid.Health > 0 and CaculateDistance(_rmMob.HumanoidRootPart.Position) < 1000 and os.time() - w < 60 and task.wait(.05) do
                    h = true
                    if string.find(_rmMob.Name, "Master") or true then
                        CombatController.Attack(_rmMob.Name)
                    else
                        pcall(sethiddenproperty, LocalPlayer, 'SimulationRadius', math.huge)
                        pcall(function()
                            _rmMob.HumanoidRootPart.CanCollide = false
                            _rmMob.Humanoid.Health = 0
                            _rmMob:BreakJoints()
                        end)
                    end
                end
    ...
    FunctionsHandler.CollectDrops:RegisterMethod("Refresh", function()
        local k = {}
        for _cdKey in ScriptStorage.Backpack do k[FruitIdToName(_cdKey)] = _cdKey end
        for _wIdx, _wChild in workspace:GetChildren() do
            if string.find(_wChild.Name, 'Fruit') and not a:FindFirstChild(_wChild.Name) and _wChild:FindFirstChild("Handle") and not k[tostring(_wChild)] and not ScriptStorage.Backpack[FruitNameToId(tostring(_wChild))] then
                FunctionsHandler.CollectDrops:Set('CurrentProgressLevel', _wChild)
                return _wChild
            end
        end
    end)
    ...
    FunctionsHandler.UtillyItemsActivitation:RegisterMethod('Refresh', function()
        ...
        if not SpecialItems then
            ...
            local function h()
                ...
                for w, w in BossesOrder do
                    local D = true
                    for _exIdx, _exName in k do if _exName == w then D = false end end
                    if D then table.insert(X, w) end
                end
                local k = #X
                for w = 1, k - 1 do
                    for D = 1, k - w do
                        local _sortA = tostring(X[D]):lower()
                        local _sortB = tostring(X[D + 1]):lower()
            ...
            print(9)
            BossesOrder = h()
            for _diName, _diData in DropItemData do
                if not ScriptStorage.Backpack[_diName] and SeaIndex == _diData.Sea then
                    if ScriptStorage.PlayerData.Level >= _diData.Level then
                        BossesOrderLevel[_diData.Boss] = _diData.Level
                        table.insert(BossesOrder, _diData.Boss)
                    end
                end
            end
            print(10)
        ...
        print(11)
        for _siIdx, _siName in SpecialItems do
            if ScriptStorage.Tools[_siName] then
                FunctionsHandler.UtillyItemsActivitation:Set('CurrentProgressLevel', _siName)
                return _siName
            end
        end
        print(12)
    ...
    FunctionsHandler.Trevor:RegisterMethod('GetFruit', function()
        for _tfKey, _tfItem in ScriptStorage.Backpack do
            if string.find(FruitIdToName(_tfItem.Name), " Fruit") then
                if _tfItem.Value and _tfItem.Value > 1000000 and _tfItem.Value < 2500000 then return _tfItem end
            end
        end
    end)
    ...
    FunctionsHandler.SoulGuitar:RegisterMethod("Refresh", function()
        if not getgenv().Config.Items.SoulGuitar then return end
    ...
    FunctionsHandler.SoulGuitar:RegisterMethod('Start', function(k)
        ...
        elseif k == 2 then
            ...
            local h = {}
            for _lzIdx, _lzMob in Services.Workspace.Enemies:GetChildren() do
                if _lzMob.Name == "Living Zombie" then table.insert(h, _lzMob) end
            end
            if #h < 6 then
    ...
    FunctionsHandler.CursedDualKatana:RegisterMethod("Refresh", function()
        if not getgenv().Config.Items.CursedDualKatana then return end
    ...
    FunctionsHandler.CursedDualKatana:RegisterMethod("GetHazeMon", function()
        local k = {}
        for _qhIdx, _qhChild in LocalPlayer.QuestHaze:GetChildren() do if _qhChild.Value > 0 then table.insert(k, _qhChild) end end
    ...
    FunctionsHandler.CursedDualKatana:RegisterMethod("DoDimension", function(k)
        ...
        repeat
            ...
            if k then
                for _ddIdx, _ddChild in k:GetChildren() do
                    if _ddChild and string.find(_ddChild.Name, "Torch") and _ddChild:FindFirstChild('ProximityPrompt') and _ddChild.ProximityPrompt.Enabled then
                        LocalPlayer.Character.HumanoidRootPart.CFrame = _ddChild.CFrame
                        _ddChild.ProximityPrompt.HoldDuration = 0
                        task.wait(1)
                        local X = game:GetService("VirtualInputManager")
                        X:SendKeyEvent(true, "E", 0, game)
                        X:SendKeyEvent(false, "E", 0, game)
                        fireproximityprompt(workspace.Map:WaitForChild(W, 10):FindFirstChild(tostring(_ddChild)).ProximityPrompt)
                    end
                    for _reIdx, _reMob in workspace.Enemies:GetChildren() do
                        local _reHrp = _reMob:FindFirstChild("HumanoidRootPart")
                        local _reHum = _reMob:FindFirstChild("Humanoid")
                        if _reHrp and _reHum and CaculateDistance(_reHrp.CFrame) < 1000 then CombatController.Attack(_reMob.Name) end
                    end
                end
    ...
    FunctionsHandler.CursedDualKatana:RegisterMethod("Start", function(k)
        ...
        elseif k[1] == 'Evil' then
            ...
            if k[2] == 1 then
                for _lbIdx, _lbNpc in game.ReplicatedStorage.NPCs:GetChildren() do
                    if _lbNpc.Name == "Luxury Boat Dealer" then
                        repeat
                            task.wait()
                            if os.time() - DoneCdkTick < 15 then return end
                            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = (_lbNpc:GetModelCFrame())
                            RealNPC = nil
                            for _rnIdx, _rnNpc in workspace.NPCs:GetChildren() do
                                if CaculateDistance(_rnNpc:GetModelCFrame(), _lbNpc:GetModelCFrame()) < 20 then
                                    RealNPC = _rnNpc
                                    break
                                end
                            end
                        until CaculateDistance(_lbNpc:GetModelCFrame()) < 5 and RealNPC
    ...
    getgenv().NotificationCallBack = (function(W)
        for _nlKey, _nlFunc in k.Listeners do
            if string.find(string.lower(W), string.lower(_nlKey)) then _nlFunc(W) end
        end
    end)
    ...
    k:RegisterNotifyListener("elite", function()
        FunctionsHandler.Yama:Set('EliteCount', Remotes.CommF_:InvokeServer("EliteHunter", "Progress"))
        alert("[Kyzen Kaitun ] ", "Elite defeated: " .. tostring(FunctionsHandler.Yama:Get("EliteCount") or 'n/a'))
    end)
    ...
    local _origNotify
    _origNotify = hookfunction(require(game.ReplicatedStorage.Notification).new, function(W, h)
        v21 = tostring(tostring(W or '') .. tostring(h or "")) or ""
        getgenv().NotificationCallBack(v21)
        return _origNotify(W, h)
    end)
    ...
    function GetServers()
        ...
        for k = 1, 100, 1 do
            local _srvData = game:GetService("ReplicatedStorage"):WaitForChild("__ServerBrowser"):InvokeServer(k)
            if IfTableHaveIndex(_srvData) then
                LastServersDataPulled = os.time()
                CachedServers = _srvData
                return _srvData
            end
        end
    end
    ...
    function Hop(k, W)
        local h = GetServers()
        local X = {}
        for _srvId, _srvInfo in h do
            table.insert(X, {JobId = _srvId, Players = _srvInfo.Count, LastUpdate = _srvInfo.__LastUpdate, Region = _srvInfo.Region})
        end
        print(#X, "servers received")
        for _hopIdx = 1, #X do
            while task.wait() do
                local _randIdx = math.random(1, #X)
                ServerData = X[_randIdx]
                if ServerData then
    ...
    LowHop = function(_maxPlayers, _region)
        local _lowServers = {}
        ...
        if h and h.data then
            for _lsIdx, _lsData in next, h.data do
                if type(_lsData) == "table" and tonumber(_lsData.playing) and tonumber(_lsData.maxPlayers) and _lsData.playing < 5 and _lsData.id ~= JobId then
                    table.insert(_lowServers, 1, _lsData.id)
                end
            end
        end
        if #_lowServers > 0 then
            game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, _lowServers[math.random(1, #_lowServers)], game.Players.LocalPlayer)
        else
    ...
    setmetatable(Services, {__index = function(_s, _svc) return game:GetService(_svc) end})
    LocalPlayer = game.Players.LocalPlayer
    ...
    function RefreshTasksData()
        if _G.Stop then return end
        for _tIdx, _tName in TasksOrder do
            local h = FunctionsHandler[_tName]
            if not h.Initalized then
                if not k[_tName] then
                    print("[ Debug ] Task", _tName, "is not registered yet")
                    k[_tName] = true
                end
            else
                ...
                if k then
                    ...
                    if h and ParsingTimes > 100 then
                        CurrentTask = CurrentTask ~= _tName
                        CurrentTask = _tName
                        ScriptStorage.Interface.SetText('DebugLine', _tName)
                        X:Call(h)
                        return
                    end
                end
    ...
    J:RefreshQuest()
    RefreshInventory()
    pcall(function() Services.ReplicatedStorage.Remotes.CommE.OnClientEvent:Connect(function(...)
        local J = {...}
    ...
    function NearbyHopHandler()
        ...
        for _plIdx, _plr in a:GetPlayers() do
            local _plPos = _plr and _plr.Character and _plr.Character:FindFirstChild("HumanoidRootPart") and _plr.Character.HumanoidRootPart.Position
            if _plPos then
                local W = QueueList[_plr.Name]
                if not W then
                    QueueList[_plr.Name] = os.time()
                else
                    if os.time() - W > 30 then
                        if CaculateDistance(_plPos) < 100 then
                            Hop('nearby plr')
                            task.wait(5)
                        else
                            QueueList[_plr.Name] = nil
                        end
                    end
                end
            end
    ...
    task.spawn(function()
        task.wait(getgenv().Config.Configuration.AutoHopDelay)
        if not getgenv().Config.Configuration.AutoHop then Hop('Autohop') end
    end)
    while task.wait() do
        if getgenv().Config.Configuration.HopWhenIdle and LastIdling and os.time() - LastIdling > 300.0 then
